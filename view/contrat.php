...
if (
    ...
    if (
        ...
        $contrat = new Contrat(
            ...
            $_POST["temps_travail"],
            $_POST["salaire"],
            $_POST["typec"], // Utilisez le type de contrat déterminé ci-dessus
...
<nav class="navbar navbar-expand-lg fixed-top">
        <div class="container"> 
          ...
          <div><button  class="Sign-Up-button" style="margin-left: 20px;">Sign-Up</button></div>
...
<script>
    ...
    // Contrôle de saisie pour le formulaire CDD
    Form.addEventListener('submit', function(e) {
        ...
        var contractType = document.getElementById("typec").value.trim().toLowerCase(); // Récupérer et mettre en minuscules le type de contrat

        // Validation des champs
        var isValid = true;

        // Contrôle de saisie pour l'Employee ID
        if (employeeId.length === 0) {
            displayError("span_ID_employe", "Veuillez entrer l'Employee ID");
            isValid = false;
        } else {
            clearError("span_ID_employe");
        }

        // Contrôle de saisie pour l'Employer ID
        if (employerId.length === 0) {
            displayError("span_ID_employeur", "Veuillez entrer l'Employer ID");
            isValid = false;
        } else {
            clearError("span_ID_employeur");
        }

        // Contrôle de saisie pour le Job Title
        if (jobTitle.length === 0 || !jobTitle.match(/^[a-zA-Z]/) || jobTitle.length < 3) {
            displayError("span_Titre_poste", "Veuillez entrer un Job Title valide (commençant par une lettre et ayant au moins 3 caractères)");
            isValid = false;
        } else {
            clearError("span_Titre_poste");
        }

        // Contrôle de saisie pour les heures de travail
        if (workHours.length === 0) {
            displayError("span_temps_travail", "Veuillez entrer les heures de travail");
            isValid = false;
        } else {
            clearError("span_temps_travail");
        }

        // Contrôle de saisie pour le salaire
        if (salary.length === 0 || parseFloat(salary) <= 0) {
            displayError("span_salaire", "Veuillez entrer un salaire valide (positif)");
            isValid = false;
        } else {
            clearError("span_salaire");
        }

        // Contrôle de saisie pour la date de début
        var currentDate = new Date();
        var inputStartDate = new Date(startDate);
        if (startDate.length === 0 || inputStartDate <= currentDate) {
            displayError("span_Date_de_debut", "La date de début doit être ultérieure à la date actuelle");
            isValid = false;
        } else {
            clearError("span_Date_de_debut");
        }

        // Contrôle de saisie pour le type de contrat
        ...
        // Contrôle de saisie pour la date d'expiration
        var inputExpirationDate = new Date(expirationDate);
        if (expirationDate.length === 0 || inputExpirationDate <= inputStartDate) {
            displayError("span_Date_expiration", "La date d'expiration doit être ultérieure à la date de début");
            isValid = false;
        } else {
...
<section class="section">
      ...
      <div class="container d-flex align-items-center justify-content-center fs-1 text-white flex-culumn">
        <h1>Explorez des opportunités d'emploi sur mesure</h1>
        <img src="../img/téléchargement (2).png" width=100%>
